...
function FileCard({ file, onDelete }) {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try { await onDelete(file.public_id, file.resource_type); }
  ...
  return (
    <div className="bg-mh-paper border border-mh-line rounded-xl overflow-hidden
      ...
      <div className="h-32 sm:h-36 bg-mh-paper-2 flex items-center justify-center overflow-hidden">
        {isImage ? (
          <img
      ...
      <div className="p-2.5 sm:p-3">
        <p className="text-xs font-medium text-mh-text truncate">
          {file.public_id.split('/').pop()}
        </p>
        <p className="text-[10px] sm:text-[11px] text-mh-text-4 mt-0.5">
          {file.bytes ? `${(file.bytes / 1024).toFixed(0)} Ko` : ''}
          {file.format ? ` · ${file.format.toUpperCase()}` : ''}
        </p>
      </div>
      <div className="px-2.5 pb-2.5 sm:px-3 sm:pb-3 flex gap-2">
        ...
        <button onClick={handleDelete} disabled={deleting}
          className="shrink-0 p-1.5 text-mh-text-4 hover:text-mh-text hover:bg-mh-paper-3 rounded-lg transition-colors disabled:opacity-40">
          <Trash2 size={13} />
        </button>
      </div>
    </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm px-4"
      ...
      <div className="bg-mh-paper border border-mh-line rounded-2xl shadow-[0_24px_60px_-12px_rgba(0,0,0,.18)]
                      w-full max-w-sm p-5 sm:p-6 space-y-4 sm:space-y-5">
        ...
        <div
          ...
          className={`border-2 border-dashed rounded-xl p-6 sm:p-7 text-center cursor-pointer transition-colors ${
          ...
          {uploading ? (
            ...
            <>
              <Upload size={20} className="mx-auto text-mh-text-4 mb-2" />
              <p className="text-sm text-mh-text-3">
                Glissez ou <span className="font-medium text-mh-text">cliquez</span>
              </p>
              <p className="text-xs text-mh-text-4 mt-1">Images, PDF — 20 Mo max</p>
            </>
          )}
    ...
    <div className="space-y-4 sm:space-y-5 max-w-5xl">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <h2 className="text-lg sm:text-xl font-semibold text-mh-text tracking-tight">Médiathèque</h2>
          <p className="text-xs sm:text-sm text-mh-text-3 mt-0.5 truncate">
      ...
      <div className="-mx-4 sm:mx-0 px-4 sm:px-0 overflow-x-auto scrollbar-none">
        <div className="flex gap-1 bg-mh-paper-2 border border-mh-line rounded-xl p-1 w-fit min-w-full sm:min-w-0">
          {CATS.map(c => (
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-mh-text-4">
          ...
          <p className="font-medium text-mh-text-3 text-sm">Aucun fichier dans cette rubrique</p>
          <p className="text-xs sm:text-sm mt-1">Importez des supports via le bouton « Importer ».</p>
        </div>
      ) : (
